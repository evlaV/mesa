...
radv_amdgpu_virtual_bo_clear_mapping(struct radv_amdgpu_winsys *ws, struct radv_amdgpu_winsys_bo *bo)
{
   ...
   r = radv_amdgpu_bo_va_op(ws, 0, 0, va_size, bo->base.va, 0, 0, AMDGPU_VA_OP_CLEAR, 0, NULL);
   if (r)
      return r;
   ...
   if (bo->emulate_sparse_residency) {
      ...
      r = radv_amdgpu_bo_va_op(ws, 0, 0, va_size, low_va, 0, 0, AMDGPU_VA_OP_CLEAR, 0, NULL);
   }
                           ...
                           const struct vk_sync_wait *waits)
{
   ...
   r = radv_amdgpu_bo_va_op(ws, bo->bo_handle, bo_offset, size, parent->base.va + offset, 0, 0, AMDGPU_VA_OP_REPLACE, wait_count, waits);
   if (r)
      return r;
   ...
   if (parent->emulate_sparse_residency) {
      ...
      r = radv_amdgpu_bo_va_op(ws, bo->bo_handle, bo_offset, size, low_va + offset, 0, 0, AMDGPU_VA_OP_REPLACE, wait_count, waits);
      if (r)
         return r;
   }
